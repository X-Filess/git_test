...
static int thiz_forced_flag = 0;

int app_factory_serialization(int forced_flag)
{
    static int init_flag = 0;
    if(factory_serialization_enter(forced_flag))
    {// need serialization
        if(0 == init_flag)
        {
            init_flag = 1;
            GUI_LinkDialog(WND_FACTORY_SERIALIZATION, PATH_XML);
        }
        thiz_current_item = 0;
        thiz_forced_flag = forced_flag;
        app_create_dialog(WND_FACTORY_SERIALIZATION);
        _create_display_timer();
    }
    else
    {// need not
	    return 1;//app_create_dialog("wnd_full_screen");
    }
    return 0;
}
