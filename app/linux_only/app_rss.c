...
{ 
	...
	class0_file_Record.property=(Name_T *)GxCore_Malloc(sizeof(Name_T)*MAX_CLASS_MENU_ITEM);
	...
	class1_file_Record.property=(Name1_T *)GxCore_Malloc(sizeof(Name1_T)*MAX_CLASS_MENU_ITEM);
	...
	email_file_Record.property=(Email_property_T *)GxCore_Malloc(sizeof(Email_property_T)*MAX_EMAIL_SAVE_ITEM);
...
void exit_rss(void)
{
	...
	if(email_file_Record.property!=NULL)
	{
		GxCore_Free(email_file_Record.property);
		RSS_DEBUG("email free sucuss!\n");
	}
	if(class1_file_Record.property!=NULL)
	{
		GxCore_Free(class0_file_Record.property);
		RSS_DEBUG("class1 free sucuss!\n");
	}
	if(class0_file_Record.property!=NULL)
	{
		GxCore_Free(class1_file_Record.property);
...
int system_rss(char *cmd,char *filepath,int timeout,int retry_times,CALL_BACK_FIND_FILE timeout_file_call_back)
{
		...
		if(strlen(filepath)>=256)
			return -1;
		p=GxCore_Malloc(sizeof(CALLBACK_PARAM));
		if(p==NULL)
			return -2;
...
void  parse_file_save(char *path,void *struct_buf,Parse_File_call_back callback)
{
	...
	else
	{
	   ...
	   else
	   {
			...
			buf=GxCore_Malloc((length/2+1)*2);//分配偶数个，减少随片
			if(buf==NULL)
			{	
				fclose(file);
				return ;
