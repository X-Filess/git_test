    ...
    if(GUI_KEYDOWN ==  event->type)
    {
        switch(find_virtualkey_ex(event->key.scancode,event->key.sym))
        {
            ...
            case APPK_BACK:
                ...
                {
                    printf("file_browser_listview_keypress exit file browser_listvew\n");
                    return EVENT_TRANSFER_KEEPON;
                }
                return EVENT_TRANSFER_STOP;
                break;
            ...
            case APPK_MENU:
                ...
                if(file_browser_listview_backkey() == 2)
                {
                    printf("exit file browser_listvew\n");
                    return EVENT_TRANSFER_KEEPON;
                }
                return EVENT_TRANSFER_STOP;
...
{
    struct gxfs_hot_device* dev;//must be del

    if(listview_mode_get() == LISTVIEW_MODE_PARTITION)
    {
        dev = GxCore_HotplugGetFirst();
        /*
         * the plug out device may be not the frist dev
         */
        if(strncmp(dev->dev_name,hotplug_dev,strlen(hotplug_dev)) == 0)
        {
            HotplugPartitionDestroy(dev);
        }
        else
        {
            while( (dev = GxCore_HotplugGetNext(dev)) != NULL )
            {
                if(strncmp(dev->dev_name,hotplug_dev,strlen(hotplug_dev)) == 0 )
                {
                    HotplugPartitionDestroy(dev);
