	...
	if(GUI_KEYDOWN ==  event->type)
	{
		...
		switch(event->key.sym)
		{
			...
			case STBK_MENU:
				...
				&&(1 == app_channel_info_get_prog_lock_flag()))
				{
					s_passwd_data.cur_cursor = 0;
					GUI_SetFocusWidget(s_btn_input[s_passwd_data.cur_cursor]);
					for(index = 0; index<4;index++)
					{
						GUI_SetProperty(s_btn_input[index], "string", " ");
					}
				...
				//GUI_SetProperty(WND_PASSWD, "draw_now", "NULL");
			...
			default:
				if(_input_cursor_passwd(event->key.sym) == GXCORE_SUCCESS)
				{
					if(_set_passwd_cursor_next() == GXCORE_ERROR) //end
					{
						...
						/*this callbak function exec in the
						* app_passwd_dlg_destroy ,don't ask me why not, ^_^
						*/
						if(s_passwd_dlg.passwd_exit_cb != NULL)
						{
							s_passwd_dlg.passwd_exit_cb(&passwd_ret, s_passwd_dlg.usr_data);
...
#define WND_PASSWD_Y 190
	...
	app_create_dialog(WND_PASSWD);

	/*if (GUI_CheckDialog("wnd_channel_list") == GXCORE_SUCCESS)
	{
		GUI_SetProperty(WND_PASSWD, "format", "popup");	
	}
	else
	{
		GUI_SetProperty(WND_PASSWD, "format", "dialog");	
	}*/
