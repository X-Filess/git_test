...
#if NSCAS_SUPPORT
#define WND_ENTITLEMENT             "wnd_nstv_entitlement_history_skynet"
#define PATH_XML                    "ca/nstv/wnd_nstv_entitlement_history_skynet.xml"
#define TXT_TITLE                   "txt_nstv_entitlement_history_skynet_title"
#define LSV_ENTITLEMENT             "lsv_nstv_entitlement_history_skynet_list"
...
void app_nstv_entitlement_skynet_exec(unsigned short operator_id)
{
    static char init_flag = 0;
    if(_entitlement_init(operator_id) < 0)
    {
        printf("\nERROR, %s, %d\n", __FUNCTION__, __LINE__);
        return ;
    }
...
// for entitle notify
void app_nstv_entitle_skynet_notify(char* param)
{
...
int app_nstv_check_entitlement_skynet(void)
{
        ...
        if(operator_count > 0)
        {
            ...
            if(0 == CASCAM_Get_Property(&data_get))
            {
                ...
                for(i = 0; i < operator_info.operator_count; i++)
                {
                    ...
                    data_get.main_id = ENTITLEMENT_INFO;
                    data_get.sub_id = NSTV_OPERATOR_ENTITLEMENT_COUNT;
                    entitle_count = atoi((const char*)operator_info.operator_info[i].operator_id);// input and output
                    data_get.data_buf = (void*)&entitle_count;
                    data_get.buf_len = sizeof(unsigned short);
                    if(0 == CASCAM_Get_Property(&data_get))
                    {
...
// for UI
SIGNAL_HANDLER int On_lsv_nstv_entitlement_history_skynet_list_get_total(GuiWidget *widget, void *usrdata)
{
	return thiz_entitlement_info.info_count;
}

SIGNAL_HANDLER int On_lsv_nstv_entitlement_history_skynet_list_get_data(GuiWidget *widget, void *usrdata)
{
...
SIGNAL_HANDLER int On_wnd_nstv_entitlement_history_skynet_keypress(GuiWidget *widget, void *usrdata)
{
		...
		case GUI_KEYDOWN:
			...
			break;
                ...
                case STBK_MENU:
                {
                    ...
                    if(GUI_CheckDialog("wnd_nstv_operator_sub") != GXCORE_SUCCESS)
                    {
...
SIGNAL_HANDLER int On_wnd_nstv_entitlement_history_skynet_destroy(GuiWidget *widget, void *usrdata)
{
    _release_data();
