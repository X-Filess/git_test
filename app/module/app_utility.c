...
{
    double init_value = TIME_ZONE_VALUE;
	int value = 0;

    GxBus_ConfigGetDouble(TIME_ZONE, &init_value, TIME_ZONE_VALUE);
    if(init_value < -12)
        init_value = -12;
    if(init_value > 12)
        init_value = 12;
    ...
    if(init_value < 0)
    {
        src_sec += (unsigned int)(HOUR_TO_SEC * fabs(init_value));
    }
    else
    {
        src_sec -= (unsigned int)(HOUR_TO_SEC * fabs(init_value));
    ...
    return src_sec-HOUR_TO_SEC*value;
}
...
WeekDay app_weekday_local2gmt(WeekDay local_weekday, time_t local_day_sec)
{
    ...
    if(zone < 0)
    {
        temp_sec = local_day_sec + (unsigned int)(fabs(zone) * HOUR_TO_SEC);
    }
    else
    {
        temp_sec = local_day_sec - (unsigned int)(fabs(zone) * HOUR_TO_SEC);
...
WeekDay app_weekday_gmt2local(WeekDay gmt_weekday, time_t gmt_day_sec)
{
    ...
    if(zone < 0)
    {
        temp_sec = gmt_day_sec - (unsigned int)(abs(zone) * HOUR_TO_SEC);
    }
    else
    {
        temp_sec = gmt_day_sec + (unsigned int)(abs(zone) * HOUR_TO_SEC);
...
uint8_t time_get_MJD_from_local_time(uint16_t* p_MJD)
{
	GxTime local_time;
	struct tm *ptm = NULL;
	time_t time_sec = 0;
    ...
    *p_MJD = 14956
           + D
           + ( signed int ) ( ( Y - L ) * 365.25 )
           + ( signed int ) ( ( M + 1 + L * 12 ) * 30.6001 );
	return 0;
}
uint8_t time_get_weekday_from_MJD( uint16_t MJD, uint8_t* p_WD )
{
    *p_WD = ( ( MJD + 2 ) % 7 ) + 1;
    return 0;
}
uint8_t time_get_week_from_local_time( uint8_t* p_week )
{
    uint16_t MJD = 0;
    time_get_MJD_from_local_time(&MJD );
    time_get_weekday_from_MJD( MJD, p_week );
    return 0;
}
