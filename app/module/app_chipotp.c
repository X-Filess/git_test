...
void device_chip_otp_close(void)
{
    if(thiz_device_fd > 0)
    {
...
int device_chip_otp_read(unsigned int addr, unsigned int read_bytes, unsigned char *data)
{
    int ret = 0;
...
int device_chip_otp_write(unsigned int addr, unsigned int write_bytes, unsigned char *data)
{
    int ret = 0;
    unsigned start_addr = addr;
    if((0x7ff < addr) || (NULL == data))
